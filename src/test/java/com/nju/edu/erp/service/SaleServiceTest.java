...
public class SaleServiceTest { // 该测试为集成测试，需要用到数据库，请大家连给定的测试数据库进行测试
    ...
    public void makeSaleSheet() { // 测试销售单是否生成成功
        ...
        saleService.makeSaleSheet(userVO, saleSheetVO, null);
    ...
    public void approval_2() { // 测试二级审批
        ...
        Assertions.assertEquals(0, productDao.findById("0000000000400000").getRecentPp().compareTo(BigDecimal.valueOf(2800.00)));
        Assertions.assertEquals(0, productDao.findById("0000000000400001").getRecentPp().compareTo(BigDecimal.valueOf(3800.00)));

        Assertions.assertEquals(0, customerDao.findOneById(2).getReceivable().compareTo(BigDecimal.valueOf(4431400.00)));
    ...
    @Transactional
    @Rollback(value = true)
    public void getDetailSaleSheet(){
        SaleFilterVO saleFilterVO = SaleFilterVO.builder()
                .build();
        List<SaleDetailSheetVO> saleDetailSheetVOS = saleService.getDetailSaleSheet(saleFilterVO);
        Assertions.assertEquals(18,saleDetailSheetVOS.size());
    }

    @Test
    @Transactional
    @Rollback(value = true)
    public void getDetailSaleSheet2(){
        SaleFilterVO saleFilterVO = SaleFilterVO.builder()
                .begin(new Date())
                .build();
        List<SaleDetailSheetVO> saleDetailSheetVOS = saleService.getDetailSaleSheet(saleFilterVO);
        Assertions.assertEquals(0,saleDetailSheetVOS.size());
    }
    @Test
    @Transactional
    @Rollback(value = true)
    public void getDetailSaleSheet3(){
        SaleFilterVO saleFilterVO = SaleFilterVO.builder()
                .end(new Date())
                .build();
        List<SaleDetailSheetVO> saleDetailSheetVOS = saleService.getDetailSaleSheet(saleFilterVO);
        Assertions.assertEquals(16,saleDetailSheetVOS.size());
    }

    @Test
    @Transactional
    @Rollback(value = true)
    public void getDetailSaleSheet4(){
        SaleFilterVO saleFilterVO = SaleFilterVO.builder()
                .name("戴尔电脑")
                .operator("xiaoshoujingli")
                .build();
        List<SaleDetailSheetVO> saleDetailSheetVOS = saleService.getDetailSaleSheet(saleFilterVO);
        Assertions.assertEquals(8,saleDetailSheetVOS.size());
    }

    @Test
    @Transactional
    @Rollback(value = true)
    public void saleBusinessProcessTest(){
        BusinessProcessFilterVO filterVO = BusinessProcessFilterVO.builder()
                .end(new Date())
                .customerId(2)
                .operator("xiaoshoujingli")
                .build();
        List<SaleSheetVO> saleSheetVOS = saleService.getBusinessProcess(filterVO);
        Assertions.assertEquals(3,saleSheetVOS.size());
    }

    @Test
    @Transactional
    @Rollback(value = true)
    public void saleBusinessProcessTest2(){
        BusinessProcessFilterVO filterVO = BusinessProcessFilterVO.builder()
                .begin(new Date())
                .customerId(2)
                .operator("xiaoshoujingli")
                .build();
        List<SaleSheetVO> saleSheetVOS = saleService.getBusinessProcess(filterVO);
        Assertions.assertEquals(0,saleSheetVOS.size());
    }

    @Test
    @Transactional
    @Rollback(value = true)
    public void redFlushSaleSheet() {
        SaleSheetContentVO saleSheetContentVO = SaleSheetContentVO.builder()
                .pid("0000000000400001")
                .quantity(100)
                .unitPrice(BigDecimal.valueOf(4200))
        ...
        SaleSheetVO saleSheetVO = SaleSheetVO.builder()
                .id("XSD-20220524-00005")
                .supplier(2)
                .salesman("xiaoshoujingli")
                .state(SaleSheetState.SUCCESS)
                .operator("xiaoshoujingli")
                .rawTotalAmount(BigDecimal.valueOf(100000))
        ...
        SaleSheetVO latest = saleService.getLatest();
        Assertions.assertEquals(0,latest.getFinalAmount().compareTo(BigDecimal.valueOf(-90000)));
        Assertions.assertEquals(0,latest.getRawTotalAmount().compareTo(BigDecimal.valueOf(-100000)));
        SaleSheetContentVO latestContent = saleService.getSaleSheetContentVOS(latest.getId()).get(0);
