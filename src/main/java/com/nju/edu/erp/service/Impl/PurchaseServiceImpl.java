...
public class PurchaseServiceImpl implements PurchaseService {
    ...
    @Override
    public PurchaseSheetVO getLatest() {
        PurchaseSheetPO purchaseSheetPO = purchaseSheetDao.getLatest();
        PurchaseSheetVO purchaseSheetVO = new PurchaseSheetVO();
        BeanUtils.copyProperties(purchaseSheetPO,purchaseSheetVO);
        List<PurchaseSheetContentVO> purchaseSheetContentVOS = new ArrayList<>();
        for (PurchaseSheetContentPO sContentPO:purchaseSheetDao.findContentByPurchaseSheetId(purchaseSheetVO.getId())){
            PurchaseSheetContentVO purchaseSheetContentVO = new PurchaseSheetContentVO();
            BeanUtils.copyProperties(sContentPO,purchaseSheetContentVO);
        }
        purchaseSheetVO.setPurchaseSheetContent(purchaseSheetContentVOS);
        return purchaseSheetVO;
    }

    @Override
    public List<PurchaseSheetVO> getBusinessProcess(BusinessProcessFilterVO filterVO) {
        List<PurchaseSheetVO> res = new ArrayList<>();
        List<PurchaseSheetPO> all = purchaseSheetDao.getBusinessProcess(filterVO);
        for(PurchaseSheetPO po:all){
            PurchaseSheetVO vo = new PurchaseSheetVO();
            BeanUtils.copyProperties(po,vo);
            List<PurchaseSheetContentPO> alll = purchaseSheetDao.findContentByPurchaseSheetId(po.getId());
    ...
    @Override
    public void redFlush(PurchaseSheetVO purchaseSheetVO){
        if(purchaseSheetVO.getState()!= PurchaseSheetState.SUCCESS) throw new MyServiceException("A005","状态未完成");
        PurchaseSheetPO purchaseSheetPO = new PurchaseSheetPO();
        BeanUtils.copyProperties(purchaseSheetVO,purchaseSheetPO);
        purchaseSheetPO.setTotalAmount(purchaseSheetPO.getTotalAmount().negate());
        purchaseSheetPO.setCreateTime(new Date());
        PurchaseSheetPO latest = purchaseSheetDao.getLatest();
        String id = IdGenerator.generateSheetId(latest == null ? null : latest.getId(), "XSD");
        purchaseSheetPO.setId(id);
        purchaseSheetPO.setState(PurchaseSheetState.SUCCESS);
        List<PurchaseSheetContentPO> sContentPOList = new ArrayList<>();
        for(PurchaseSheetContentVO content : purchaseSheetVO.getPurchaseSheetContent()){
            PurchaseSheetContentPO sContentPO = new PurchaseSheetContentPO();
            BeanUtils.copyProperties(content,sContentPO);
            sContentPO.setPurchaseSheetId(id);
        ...
        purchaseSheetDao.saveBatch(sContentPOList);
        purchaseSheetDao.save(purchaseSheetPO);
    }

    @Override
    public PurchaseSheetVO redFlushCopy(PurchaseSheetVO purchaseSheetVO){
        redFlush(purchaseSheetVO);
        PurchaseSheetVO res = new PurchaseSheetVO();
        BeanUtils.copyProperties(purchaseSheetVO,res);
        return res;
    }

    @Override
    public void copyIn(PurchaseSheetVO purchaseSheetVO){
        PurchaseSheetPO purchaseSheetPO = new PurchaseSheetPO();
        BeanUtils.copyProperties(purchaseSheetVO,purchaseSheetPO);
        purchaseSheetPO.setTotalAmount(purchaseSheetPO.getTotalAmount().negate());
        purchaseSheetPO.setCreateTime(new Date());
